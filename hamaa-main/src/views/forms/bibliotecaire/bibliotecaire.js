...
const Livre = () => {
  ...
  useEffect(() => {
    const fetchLivres = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/getbiblio');
  ...
  const handleDeleteLivre = async (livreId) => {
    try {
      await axios.delete(`http://localhost:5000/api/deletebiblio/${livreId}`);
  ...
  const handleEmprunter = async (livreId) => {
    ...
    if (!utilisateur) {
      console.error('Utilisateur object not found in localStorage');
      return;
    }
    if (!utilisateur.id) {
      console.error('Utilisateur ID not available');
      return;
    }
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/demande-emprunt/${userId}/${livreId}`);
                        ...
                        <tr key={index}>
                          ...
                          <td>
                            <CButton onClick={() => handleDeleteLivre(livre._id)}>
                              <FontAwesomeIcon icon={faTrash} style={{ marginRight: '10px' }} title="Supprimer" />
                            </CButton>
                            <Link to={`/forms/Updatebibliotecaire/${livre._id}`}>
                              <FontAwesomeIcon icon={faPenSquare} title="Modifier" />
                            </Link>
                            <Link to={`/Emprunter/${livre._id}`}>
                              <CButton title="Emprunter">Emprunter</CButton>
                            </Link>
                          </td>
                        </tr>
                      ))}

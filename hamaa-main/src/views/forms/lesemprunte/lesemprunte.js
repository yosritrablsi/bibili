...
const DemandeListe = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/getdemande');
  ...
  const handleDeleteDemande = async (demandeId) => {
    try {
      await axios.delete(`http://localhost:5000/api/deletedemande/${demandeId}`);
  ...
  const handleAcceptDemande = async (demandeId) => {
    try {
      await axios.post(`http://localhost:5000/api/accepterDemande/${demandeId}`);
      setDemandes(demandes.filter((demande) => demande._id !== demandeId));
